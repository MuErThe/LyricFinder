...
const Lyrics = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://github.com/Rob--W/cors-anywhere/http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${props.match.params.id}&apikey=${process.env.REACT_APP_MM_KEY}`
      )
      .then((res) => {
        ...
        return axios.get(
          `https://github.com/Rob--W/cors-anywhere/http://api.musixmatch.com/ws/1.1/track.get?track_id=${props.match.params.id}&apikey=${process.env.REACT_APP_MM_KEY}`
        );
      })
      .then((res) => {
